...
> **Status: rascunho.** Redigido com apoio de IA a partir dos artefatos versionados do repositório (`BackEnd/Treinamento_IA/models/*.json`), para atender às recomendações do orientador sobre a seção de Resultados e Discussão. A equipe deve revisar, reescrever com a própria voz e declarar o uso de IA conforme as normas da instituição. Todo número abaixo aponta para a chave do JSON de origem; as figuras são geradas por `BackEnd/Treinamento_IA/figuras_tcc.py` e se atualizam sozinhas quando os JSONs mudarem.
