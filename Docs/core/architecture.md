...
|---|---|---|
| Grafo + enriquecimento | `BackEnd/API/main.py`, `graph_enrichment.py` | Carrega o grafo (cache graphml), filtra vias não dirigíveis, anexa velocidade livre e o vínculo de cada aresta com a via monitorada mais próxima (BallTree, raio de 500 m). |
| Modelo LIA 2.1 | `lia_inference.py`, `Treinamento_IA/models/` | Prevê a razão de congestionamento por via; o tempo em segundos sai do comprimento real de cada aresta. |
| Cache de recência | `recencia_cache.py` | Última observação real por via (`razao_lag1`, `delta_min_lag1`). Faz merge (a leitura ao vivo vence o banco) e aplica backoff quando o Supabase cai. |
...
O coletor (`BackEnd/Servidor/services/traffic_collector.py`, pausado) segue a mesma política: cooldown graduado e janela de 24 h para cota.
...
`silver.py` puxa `historico_trafego` do Supabase para parquet → `features.py` (perfis via×hora×dia, recência) → `train.py` (XGBoost, TimeSeriesSplit de 5 folds, métricas gerais e do subconjunto congestionado) → artefatos `.pkl` (fora do git) + `*_metadata.json` (versionado). `calibrar_transfer.py` gera a curva isotônica; `benchmark_lstm_xgboost.py` e `validar_fase3.py` produzem as evidências da tese. `figuras_tcc.py` gera as figuras do texto em `Docs/figuras/`.
