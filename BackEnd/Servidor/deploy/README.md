...
Do jeito mais simples — copiar a pasta `BackEnd/` inteira (API, Servidor,
Treinamento_IA) para a VM, via `scp -r` ou `git clone` de um repositório
privado. Não copiar `.env` nem `tomtom_keys.json` pelo git se o repo for
público — transferir esses dois arquivos separadamente (scp direto) para
`Servidor/config/` e `API/../Servidor/config/` conforme o caminho que os
scripts já esperam.

```bash
# na VM, depois do código estar em ~/Routify/BackEnd/
...
`API/requirements.txt`). Numa VM ARM64, é preciso confirmar que o PyPI tem
...
sudo cp ~/Routify/BackEnd/Servidor/deploy/routify-collector.service \
        /etc/systemd/system/
