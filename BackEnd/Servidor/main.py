...
# Treinamento_IA e API são pacotes irmãos de Servidor/ dentro de BackEnd/ —
# validar_fase3.py já assume rodar a partir de dentro de Treinamento_IA/, e
# ele mesmo insere API/ no sys.path. Só falta o próprio Treinamento_IA/ aqui.
TREINAMENTO_IA_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'Treinamento_IA')
sys.path.insert(0, TREINAMENTO_IA_DIR)
...
def run_fase3_job() -> None:
    ...
    cada chamada (ver validar_fase3.montar_ambiente_lia) — o pico de RAM
    ...
    """
    try:
        import validar_fase3
        asyncio.run(validar_fase3.run())
