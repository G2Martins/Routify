...
cd ml && python publish_metrics.py                       # métricas da LIA → lia_treinos/lia_analises (painel ADM)
...
- **Ainda aberto:**
  - `/route` e `/search` aceitam chamada anônima, sem rate-limit por cliente. A única barreira contra queimar a cota da TomTom é o teto global `TOMTOM_MAX_CHAMADAS_MIN` (120/min);
  - `/eventos` tem limite em memória (60/min por usuário), que zera em restart e não vale entre réplicas.
...
- **CONFIRMADO em 2026-09-23 (advisor nível ERROR):** `historico_trafego`, `vias_monitoradas` e `malha_completa` estão **sem RLS**, e o `anon` tem SELECT/INSERT/DELETE. Qualquer um com a chave pública do app lê e apaga o dataset via PostgREST.
  ...
  - **Pendente: aplicar** junto com a `20260907000000_thesis_validation.sql` (Pedro, nunca aplicada: sem ela o feedback de tempo real falha) e a `20260923010000_usage_tracking_admin.sql`, nessa ordem. Como aplicar e como promover admin: [docs/core/rodar-local.md §1](docs/core/rodar-local.md).
